// src/GroupSettings.jsx — 특정 그룹의 설정 (그룹 탭에서 ⚙️로 진입)
  ...
  async function saveMyGroupName() {
    ...
    if (useGroupName) {
      ...
      const clash = (members || []).some(m =>
        m.user_id !== user.id && (m.display_name || '').trim().toLowerCase() === wanted
      )
      if (clash) { flash('이 그룹에 이미 같은 이름이 있어요. 다른 이름을 써주세요'); return }
    }
    setBusy(true)
    const payload = useGroupName
      ? { display_name: groupName.trim().slice(0, 12), color: groupColor || '#ff4d5e' }
  ...
  return (
    <div style={S.app}>
      <div style={S.top}>
        ...
        <div style={S.title}>{group.name} 설정</div>
        <div style={{ width: 32 }} />
      </div>
      ...
      <div style={S.body}>
        ...
        <div style={S.card}>
          <div style={S.toggleRow}>
            ...
            <button onClick={() => setUseGroupName(v => !v)} style={{ ...S.switch, background: useGroupName ? '#13bca4' : 'var(--mp-line2)' }}>
          ...
          {useGroupName && (
            <>
              ...
              <div style={{ display: 'flex', gap: 10 }}>
                {COLORS.map(c => (
                  <button key={c} onClick={() => setGroupColor(c)} style={{ width: 32, height: 32, borderRadius: '50%', background: c, border: (groupColor || '#ff4d5e') === c ? '3px solid var(--mp-ink)' : '3px solid var(--mp-card)', cursor: 'pointer', boxShadow: '0 2px 6px rgba(0,0,0,.15)' }} />
                ))}
              </div>
            </>
          )}
...
const S = {
  ...
  title: { fontWeight: 700, fontSize: 17, maxWidth: 260, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' },
  body: { padding: 16 },
