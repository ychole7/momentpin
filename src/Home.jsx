...
export default function Home({ user, group, profileVersion, isActive, onMembersLoaded }) {
  ...
  useEffect(() => {
    if (tab !== 'map') {
      // 지도 탭을 떠나면 기존 Leaflet 인스턴스를 깨끗이 제거
      if (mapRef.current) { try { mapRef.current.remove() } catch {} mapRef.current = null }
      markersRef.current = []
      return
    }

    // iOS Safari / PWA에서 Leaflet CDN이 늦게 로드되는 경우를 대비해
    // window.L이 준비될 때까지 짧게 재시도한다. 한 번 실패하고 지도가
    // 영원히 사라지는 문제를 방지하는 핵심 복구 코드.
    let stopped = false
    let tries = 0
    let timer = null
    const boot = () => {
      if (stopped) return
      if (window.L && mapBoxRef.current) {
        initMap()
        return
      }
      if (tries++ < 50) timer = setTimeout(boot, 100)
    }
    boot()
  ...
  function initMap() {
    ...
    // 같은 DOM에 이미 정상적인 지도가 있으면 재생성하지 않는다.
    if (mapRef.current && mapRef.current.getContainer && mapRef.current.getContainer() === el) {
      try { mapRef.current.invalidateSize(false) } catch {}
      drawPins()
      return
    }

    // React 재렌더/탭 전환 뒤 예전 Leaflet 인스턴스가 남아 있으면 제거
    if (mapRef.current) {
